...
record GamingHistory(List<DiffAndWithWho> hist) implements JsonConvertable {
  ...
  public JsonObject toJson() {
    return new JsonObject().put("history", hist.stream().map(JsonConvertable::toJson).toList());
  }
  ...
  public GamingHistory clean() {
    return new GamingHistory(hist.stream().filter(diff -> {
      Diff d = diff.diff();
  ...
  public static GamingHistory fromJson(JsonObject entries) {
    return new GamingHistory(new ArrayList<>(entries.getJsonArray("history")
      .stream()
