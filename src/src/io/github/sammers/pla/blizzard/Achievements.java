...
public record Achievements(Long totalQuantity, Long totalPoints, Set<Achievement> achievements,
  TitlesHistory titlesHistory) implements JsonConvertable {
  public static Achievements parse(JsonObject achievements) {
    ...
    return new Achievements(achievements.getLong("total_quantity"), achievements.getLong("total_points"), set,
      titlesHistory);
  }
  ...
  public static Achievements fromJson(JsonObject json) {
    ...
    return new Achievements(json.getLong("total_quantity"), json.getLong("total_points"),
  ...
  public JsonObject toJson() {
    return new JsonObject().put("total_quantity", totalQuantity)
  ...
  private static Pattern BlitzRankOnePattern = Pattern.compile("^(\\w+) (Marshal|Warlord): ([\\w\\s]+) Season (\\d+)$");
  private static Pattern HeroPattern = Pattern.compile("^Hero of the (Alliance|Horde): (.+)$");
  private static Set<String> ArenaRanks = Set.of("Gladiator", "Duelist", "Rival", "Challenger", "Legend", "Elite",
    "Combatant", "Strategist");
  ...
  private static boolean isPvpAchievement(Achievement achievement) {
    ...
    Matcher blitzR1 = BlitzRankOnePattern.matcher(name);
    Matcher hero = HeroPattern.matcher(name);
    Matcher newLowRanks = NewLowRanks.matcher(name);
    Matcher oldLowRanks = OldLowRanks.matcher(name);
    return (oldR1.matches() || newR1.matches() || shuffleR1.matches() || blitzR1.matches() || hero.matches()
      || newLowRanks.matches() || oldLowRanks.matches()) && achievement.completedTimestamp() != null;
  }

  private static Map<String, Long> rankToLong = Map.of("r1_3s", 120L, "r1_shuffle", 110L, "Gladiator", 100L, "Legend",
    90L, "Strategist", 85L, "Elite", 80L, "Duelist", 70L, "Rival", 60L, "Challenger", 50L, "Combatant", 40L);
  ...
  private static TitlesHistory calculateTitlesHistory(Set<Achievement> achievements) {
    ...
    achievements.stream().forEach(achievement -> {
      ...
      Matcher blitzR1 = BlitzRankOnePattern.matcher(achievement.name());
      if (blitzR1.find()) {
        String rank = "r1_3s";
        String expansion = blitzR1.group(3);
        Long seasonNumber = Long.parseLong(blitzR1.group(4));
        updateSeasons(expansions, achievement, rank, expansion, seasonNumber);
      }
    ...
    expansions.forEach((expansion, expansionMap) -> {
      Long maxTimestamp = expansionMap.values()
        .stream()
        .map(Pair::getValue1)
    ...
    return new TitlesHistory(expansions.keySet()
      .stream()
  ...
  private static void updateSeasons(Map<String, Map<Long, Pair<String, Season>>> seasons, Achievement achievement,
    String rank, String expansion, Long seasonNumber) {
