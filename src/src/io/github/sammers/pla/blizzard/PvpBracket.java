...
public record PvpBracket(String bracketType, Long rating, Long won, Long lost, Long rank, Long seasonMaxRating,
  Long seasonMaxRatingAchievedTimestamp, Long maxRating, Long maxRatingAchievedTimestamp, Boolean isRankOneRange,
  GamingHistory gamingHistory) implements JsonConvertable {
  public static PvpBracket parse(JsonObject wowApiBracket, Optional<PvpBracket> prevBracket, Long rank,
    Long bracketRankOneCutoff, boolean thisSsnData) {
    ...
    GamingHistory gamingHistory = prevBracket.map(PvpBracket::gamingHistory)
      .orElse(new GamingHistory(new ArrayList<>()));
    if (!thisSsnData) {
      rating = 0L;
      won = 0L;
      lost = 0L;
    ...
    return new PvpBracket(type, rating, won, lost, rank, seasonMaxRating, seasonMaxRatingAchievedTimestamp, maxRating,
      maxRatingAchievedTimestamp, isRankOneRange, gamingHistory);
  }

  public static PvpBracket fromJson(JsonObject entries) {
    return new PvpBracket(entries.getString("bracket_type"), entries.getLong("rating"), entries.getLong("won"),
      entries.getLong("lost"), entries.getLong("rank"),
      ...
      Optional.ofNullable(entries.getJsonObject("gaming_history"))
        .map(GamingHistory::fromJson)
        .orElse(new GamingHistory(new ArrayList<>())));
  }
