...
public record Realms(Map<Integer, Realm> idToRealm) implements JsonConvertable {
  ...
  public static Realms fromBlizzardJson(String realRegion, JsonObject index, List<JsonObject> values) {
    ...
    values.stream().flatMap(json -> json.getJsonArray("realms").stream()).map(JsonObject.class::cast).forEach(json -> {
  ...
  public JsonObject toJson() {
    return new JsonObject().put("realms", idToRealm.values().stream().map(Realm::toJson).toList());
  }
