...
public record Snapshot(List<Character> characters, Long timestamp, String region, String dateTime) implements Resp {
  ...
  public JsonObject toJson(Long page) {
    List<JsonObject> chars = characters.stream()
      .skip((page - 1) * 100L)
      .limit(100)
      .map(JsonConvertable::toJson)
      .toList();
    return new JsonObject().put("characters", new JsonArray(chars))
  ...
  public static Snapshot fromJson(JsonObject entries) {
    ...
    String format = Main.DATA_TIME.format(zonedDateTime);
    return new Snapshot(
      entries.getJsonArray("characters").stream().map(x -> (JsonObject) x).map(Character::fromJson).toList(), ts,
      entries.getString("region"), format);
  }
  ...
  public void predictCutoffs(String bracket, Cutoffs cutoffs) {
    ...
    if (bracket.equals(THREE_V_THREE)) {
      ...
      for (int i = 0; i < characters.size() && countedAlready.size() < spots; i++) {
        ...
        Integer petHash = last.pethash()
          .map(p -> p == -1 ? finalLast.fullNameWSpec().hashCode() : p)
      ...
      for (Character ch : characters) {
        ...
        Integer petHash = ch.pethash()
          .map(p -> p == -1 ? ch.fullNameWSpec().hashCode() : p)
  ...
  public Snapshot applyCutoffs(String bracket, Cutoffs cutoffs) {
    ...
    if (bracket.equals(THREE_V_THREE)) {
      ...
      return applyTraditionalCutoff("ARENA_3v3", cutoff, cutoffs, false);
    } else if (bracket.equals(RBG)) {
      Long cutoff = cutoffs.battlegrounds("ALLIANCE");
      if (cutoff == null) {
        return this;
      }
      return applyTraditionalCutoff("BATTLEGROUNDS/alliance", cutoff, cutoffs, true);
    } else if (bracket.equals(SHUFFLE)) {
      return applySoloCutoffs("SHUFFLE/", cutoffs::shuffle, cutoffs);
    } else if (bracket.equals(BLITZ)) {
      return applySoloCutoffs("BLITZ/", cutoffs::blitz, cutoffs);
    }
    return this;
  }

  private Snapshot applyTraditionalCutoff(String bracketKey, long cutoff, Cutoffs cutoffs, boolean copyForHorde) {
    AtomicLong charsWithCutoff = new AtomicLong(0);
    List<Character> chars = characters.stream().map(ch -> {
      if (ch.rating() >= cutoff) {
    ...
    Map<Integer, List<Character>> petHashes = chars.stream()
      ...
      .collect(Collectors.groupingBy(
        ch -> ch.pethash().isEmpty() || ch.pethash().get() == -1 ? ch.fullNameWSpec().hashCode() : ch.pethash().get()));
    cutoffs.setSpotCount(bracketKey, charsWithCutoff.intValue());
    cutoffs.setSpotWithNoAlts(bracketKey, petHashes.size());
    if (copyForHorde) {
  ...
  private Snapshot applySoloCutoffs(String prefix, Function<String, Long> cutoffProvider, Cutoffs cutoffs) {
    Map<String, Long> specCodeAndSpotCount = new HashMap<>();
    Map<String, Map<Integer, List<Character>>> petHashes = new HashMap<>();
    List<Character> chars = characters.stream().map(ch -> {
      String fullSpec = ch.fullSpec();
      String specCode = Cutoffs.specCodeByFullName(fullSpec);
      Long cutoff = cutoffProvider.apply(specCode);
      if (cutoff == null) {
        return ch;
      }
      if (ch.rating() >= cutoff) {
    ...
    specCodeAndSpotCount.entrySet().forEach(entry -> {
      cutoffs.setSpotWithNoAlts(prefix + entry.getKey(), petHashes.get(entry.getKey()).size());
      cutoffs.setSpotCount(prefix + entry.getKey(), entry.getValue().intValue());
    });
    return new Snapshot(chars, timestamp(), region(), dateTime());
  }
  ...
  /**
   * Transforms this Snapshot into a SnapshotMLExt by calculating unique positions
   * (ranks without alts) for each character using the character cache.
   *
   * @param cache
   *          The character cache to look up alt information
   * @return SnapshotMLExt with calculated posUniq values per character
   */
  public SnapshotMLExt mlExt(CharacterCache cache) {
    return SnapshotMLExt.fromSnapshot(this, cache);
  }
}
