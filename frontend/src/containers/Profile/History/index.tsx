...
const GamingHistory = ({ player }: { player: IPlayer }) => {
  ...
  const bracket = useMemo(() => {
    ...
    const search = bracketsList.includes(active_bracket_name) ? active_bracket_name : `SHUFFLE-${active_bracket_name}`;
  ...
  return (
    <div className="flex flex-col py-2 md:px-3 border border-solid border-[#37415180] rounded-lg bg-[#030303e6] ">
      <div className="flex justify-between items-center px-3 md:px-0">
        ...
        <BracketTabs player={player} onChange={handleChange} active_bracket_name={active_bracket_name} />
      </div>
      <hr className="h-px md:mb-2 bg-[#37415180] border-0" />
      <Table
        columns={tableColumns(player, active_bracket_name, breakpoints === "sm")}
        records={records}
