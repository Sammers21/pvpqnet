 ...
 * @returns colored box
 */
export const ActivityBox = ({ maxIntensity = 5, activity = [], date = undefined,year = undefined,lookingYear = undefined}) => {
  let hoverText;
  let intensity = 0;
    if (year === lookingYear || year === 0){
      ...
      if (date === undefined){
        ...
        intensity = Math.min(maxIntensity, Math.round((gamesPlayedByActivityArray(activity) / maxIntensity) * 4))
